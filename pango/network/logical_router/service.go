...
type response struct {
	Members []member `xml:"result>logical-router>member"`
}

type request struct {
	XMLName xml.Name `xml:"logical-router"`
...
func (s *Service) ImportToLocation(ctx context.Context, loc Location, vsys string, name string) error {
	...
	xpath = append(xpath, []string{"vsys", util.AsEntryXpath(vsys), "import", "network", "logical-router"}...)
