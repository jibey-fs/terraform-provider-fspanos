...
func (o Location) XpathWithComponents(vn version.Number, components ...string) ([]string, error) {
...

	{
		...
		if !strings.HasPrefix(component, "entry[") || !strings.HasSuffix(component, "]") {
			return nil, errors.NewInvalidXpathComponentError(fmt.Sprintf("VirtualRouter must be formatted as entry: %s", component))
		}
	}
	{
	...
	ans = append(ans, "virtual-router")
	ans = append(ans, components[0])
	ans = append(ans, "routing-table")
	ans = append(ans, "ip")
	ans = append(ans, "static-route")
